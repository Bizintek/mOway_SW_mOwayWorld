...
{
    public class AssignLineGraphic : GraphModule
    {
        public override bool Selected
        {
            ...
            set
            {
                if (this.selected != value)
                {
                    this.selected = value;
                    if (this.selected)
                        this.Surface.Blit(new Surface(AssignLine.GraphicIconSelected));
                    else
                    {
        ...
        public AssignLineGraphic(string key)
            : base(key)
        {
        ...
        public AssignLineGraphic(string key, XmlElement elementData, System.Collections.Generic.SortedList<string, Variable> variables)
            : base(key)
        {
            this.needInit = System.Convert.ToBoolean(AssignLine.NeedInit);
            this.Surface.Blit(new Surface(AssignLine.GraphicIcon));
            foreach (XmlElement nodo in elementData)
            {
                switch (nodo.Name)
                {
                    case "position":
                        ...
                        break;
                    case "properties":
                        this.element = new AssignLineAction(key, nodo, variables);
                        break;
                    case "previous":
                        break;
                    case "next":
        ...
        {
            return new AssignLineGraphic(this.key, (AssignLineAction)this.element.Clone(), this.Center);
        }
    }
}
