...
{
    public class PlaySoundGraphic : GraphModule
    {
        public override bool Selected
        {
            ...
            set
            {
                if (this.selected != value)
                {
                    this.selected = value;
                    if (this.selected)
                        this.Surface.Blit(new Surface(PlaySound.GraphicIconSelected));
                    else
                    {
        ...
        public PlaySoundGraphic(string key)
            : base(key)
        {
        ...
        public PlaySoundGraphic(string key, XmlElement elementData, System.Collections.Generic.SortedList<string, Variable> variables)
            : base(key)
        {
            this.needInit = System.Convert.ToBoolean(PlaySound.NeedInit);
            this.Surface.Blit(new Surface(PlaySound.GraphicIcon));
            foreach (XmlElement nodo in elementData)
            {
                switch (nodo.Name)
                {
                    case "position":
                        ...
                        break;
                    case "properties":
                        this.element = new PlaySoundAction(key, nodo, variables);
                        break;
                    case "previous":
                        break;
                    case "next":
        ...
        {
            return new PlaySoundGraphic(this.key, (PlaySoundAction)this.element.Clone(), this.Center);
        }
