...
{
    public class CompareTimeFactory : iActionFactory
    {
        #region Attributes
        ...
        public string Category { get { return CompareTime.Category; } }

        #endregion

        public CompareTimeFactory()
        {
            this.key = CompareTime.Key;
        }

        public Tool GetToolAction()
        {
            return new CompareTimeTool(this.key);
        }
        ...
        public GraphElement GetGraphAction(string key)
        {
            if (this.key != key)
                throw new ActionException("Key is not correct");
            return new CompareTimeGraphic(this.key);
        }
        ...
        public GraphElement GetGraphAction(string key, XmlElement elementData, System.Collections.Generic.SortedList<string, Variable> variables)
        {
            if (this.key != key)
                throw new ActionException("Key is not correct");
            return new CompareTimeGraphic(this.key, elementData, variables);
        }
        ...
        public Element GetAction(string key)
        {
            if (this.key != key)
                throw new ActionException("Key is not correct");
            return new CompareTimeAction(key);
        }
        ...
        public ActionForm GetActionForm(Element element)
        {
            if (this.key != element.Key)
                throw new ActionException("Key is not correct");
            return new CompareTimeForm((CompareTimeAction)element);
        }
        ...
        public ActionPanel GetActionPanel(Element element)
        {
            if (this.key != element.Key)
                throw new ActionException("Key is not correct");
            return new CompareTimePanel((CompareTimeAction)element);
        }
    }
}