...
{
    public class ResetGraphic : GraphModule
    {
        public override bool Selected
        {
            ...
            set
            {
                if (this.selected != value)
                {
                    this.selected = value;
                    if (this.selected)
                        this.Surface.Blit(new Surface(Reset.GraphicIconSelected));
                    else
                    {
        ...
        public ResetGraphic(string key)
            : base(key)
        {
        ...
        public ResetGraphic(string key, XmlElement elementData, System.Collections.Generic.SortedList<string, Variable> variables)
            : base(key)
        {
            this.needInit = System.Convert.ToBoolean(Reset.NeedInit);
            this.Surface.Blit(new Surface(Reset.GraphicIcon));
            foreach (XmlElement nodo in elementData)
            {
                switch (nodo.Name)
                {
                    case "position":
                        ...
                        break;
                    case "properties":
                        this.element = new ResetAction(key, nodo);
                        break;
                    case "previous":
                        break;
                    case "next":
        ...
        {
            return new ResetGraphic(this.key, (ResetAction)this.element.Clone(), this.Center);
        }
    }
}
